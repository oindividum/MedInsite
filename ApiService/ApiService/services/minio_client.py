    ...
    async def upload_data(
        ...
        try:
            ...
            if hasattr(data, 'seek') and hasattr(data, 'tell'):
                # Для UploadFile, BytesIO и других file-like объектов
                current_pos = data.tell()
                data.seek(0, 2)  # в конец
                length = data.tell()
                data.seek(current_pos)  # восстанавливаем позицию
            else:
                # Только если это "обычный" объект с len
                if hasattr(data, '__len__'):
                    length = len(data)
                else:
                    raise ValueError("Не удалось определить размер данных")
